...
#include "eidolon/gfx/helper.hpp"
...
void cmd_transition_image_layout(context_t &context,
  ...
  default:
    ...
    eidolon_error("not handled transition");
    break;
  }
...
handle_image_t load_image_from_path_instant(
  ...
  eidolon_trace("{} image loaded, width: {} height: {} device memory: {}",
...
handle_buffer_t create_buffer_staged(context_t &context,
                                     ...
                                     size_t size) {
  eidolon_assert(size <= config.vk_size, "copying more than allocated");
...
static void checkVkResult(VkResult error) {
  if (error == 0)
    return;
  eidolon_error("imgui error: {}", uint64_t(error));
  if (error < 0)
    abort();
}
