#include "eidolon/gfx/context.hpp"

#include <vulkan/vulkan_core.h>

#include "VkBootstrap.h"
#include "glm/fwd.hpp"
#include "eidolon/core/core.hpp"
#include "eidolon/core/logger.hpp"
#include "eidolon/core/window.hpp"
...
inline void diagnose_if_needed(slang::IBlob *diagnostics_blob) {
  if (diagnostics_blob) {
    eidolon_error("{}", reinterpret_cast<const char *>(
...
void update_descriptor_set_t::commit() {
  ...
  vk_writes.clear();
  eidolon_trace("updated descriptor set {}", handle);
}
...
struct volk_initializer_t {
  volk_initializer_t() {
    ...
    if (volkInitialize() != VK_SUCCESS) {
      eidolon_error("Failed to initialize volk");
      std::terminate();
    }
  }
};
...
context_t::~context_t() {
  ...
  for (auto &[handle, timer] : _timers) {
    eidolon_trace("forgot to clear command pool with handle: {}", handle);
    vkDestroyQueryPool(_vkb_device, timer, nullptr);
  }
  for (auto &[handle, commandbuffer] : _commandbuffers) {
    eidolon_trace("forgot to clear commandbuffer with handle: {}", handle);
  ...
  for (auto &[handle, sampler] : _samplers) {
    eidolon_trace("forgot to clear sampler with handle: {}", handle);
    vkDestroySampler(_vkb_device, sampler, nullptr);
  }
  for (auto &[handle, buffer] : _buffers) {
    eidolon_trace("forgot to clear buffer with handle: {}", handle);
  ...
  for (auto &[handle, swapchain] : _swapchains) {
    eidolon_trace("forgot to clear swapchain with handle: {}", handle);
...
debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT      vk_message_severity,
  ...
  switch (vk_message_severity) {
    ...
    case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
      ...
      break;
    default:
      eidolon_error("unknown message severity");
      std::terminate();
  }
  return VK_FALSE;
}
...
void context_t::create_instance() {
  ...
  eidolon_trace("created instance");
}
...
void context_t::create_device() {
  ...
  vkb_physical_device_selector.set_surface(vk_temp_surface);
  {
    ...
    eidolon_trace("{}", _vkb_physical_device.name);
  }
...
void context_t::create_allocator() {
  ...
  eidolon_trace("created allocator");
}
...
void context_t::create_descriptor_pool() {
  ...
  eidolon_trace("created descriptor pool");
}
...
handle_swapchain_t context_t::create_swapchain(const core::window_t &window) {
  ...
  handle_swapchain_t handle =
      utils::create_and_insert_new_handle<handle_swapchain_t>(_swapchains,
                                                              swapchain);
  eidolon_trace("created swapchain");
  return handle;
}
...
std::optional<uint32_t> context_t::get_swapchain_next_image_index(
  ...
  uint32_t next_image;
  {
    ...
    if (vk_result == VK_ERROR_OUT_OF_DATE_KHR) {
      eidolon_warn("acquire next image failed");
      return std::nullopt;
    }
...
bool context_t::present_swapchain(
...

  {
    ...
    if (vk_result == VK_ERROR_OUT_OF_DATE_KHR) {
      eidolon_warn("queue present failed: OUT OF DATE KHR");
      return false;
    }
    if (vk_result == VK_SUBOPTIMAL_KHR) {
      eidolon_warn("queue present failed: SUBOPTIMAL KHR");
      return false;
    }
    check(vk_result == VK_SUCCESS, "Failed to present");
  }
  if (vk_result == VK_SUBOPTIMAL_KHR) {
    eidolon_warn("some internal shii");
    return false;
  }
  check(vk_result == VK_SUCCESS, "Failed to present");
  return true;
...
handle_buffer_t context_t::create_buffer(const config_buffer_t &config) {
  ...
  if (config.debug_name != "") {
    ...
    eidolon_trace("created buffer {}", buffer.config.debug_name);
  } else {
    eidolon_trace("created buffer");
  }
  return handle;
}
...
handle_sampler_t context_t::create_sampler(const config_sampler_t &config) {
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created sampler {}", sampler.config.debug_name);
  } else {
    eidolon_trace("created sampler");
  }
  return handle;
}
...
handle_image_t context_t::create_image(const config_image_t &config) {
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created image {}", image.config.debug_name);
  } else {
    eidolon_trace("created image");
  }
  return handle;
}
...
handle_image_view_t context_t::create_image_view(
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created image view {}", image_view.config.debug_name);
  } else {
    eidolon_trace("created image view");
  }
  return handle;
}
...
handle_descriptor_set_layout_t context_t::create_descriptor_set_layout(
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created descriptor set layout {}",
...
handle_descriptor_set_t context_t::allocate_descriptor_set(
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("allocated descriptor set {}",
...
handle_pipeline_layout_t context_t::create_pipeline_layout(
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created pipeline layout {}",
...
handle_shader_t context_t::create_shader(const config_shader_t &config) {
  ...
  //     eidolon_error("{}", compileRequest->getDiagnosticOutput());
  //     std::terminate();
  // }

  // if (SLANG_FAILED(compileRequest->compile())) {
  //     eidolon_error("{}", "somthing fucked up");
  //     std::terminate();
  // }
  ...
  switch (config.type) {
    ...
    case shader_type_t::e_compute:
      ...
      break;
    default:
      eidolon_error("unknown shader type");
      std::terminate();
  }
  utils::diagnose_if_needed(diagnosticBlob);
  ...
  for (uint32_t i = 0; i < parameter_count; i++) {
    ...
    eidolon_trace("{} {}", parameter->getName(), index);
  }
  ...
  if (!config.is_code) eidolon_trace("reading file {}", config.code_or_path);
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created shader {}", shader.config.debug_name);
  } else {
    eidolon_trace("created shader");
  }
  return handle;
}
...
handle_pipeline_t context_t::create_compute_pipeline(
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created compute pipeline {}", pipeline.config.debug_name);
  } else {
    eidolon_trace("created compute pipeline");
  }
  return handle;
}
...
handle_pipeline_t context_t::create_graphics_pipeline(
  ...
  for (auto &handle_shader : config.handle_shaders) {
    ...
    switch (shader.config.type) {
      ...
      case shader_type_t::e_vertex:
        ...
        break;
      default:
        eidolon_error("shader is neither vertex nor fragment");
        std::terminate();
    }
  ...
  if (config.debug_name != "") {
    ...
    vk_debug_utils_object_name_info.pObjectName =
        pipeline.config.debug_name.data();
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created graphics pipeline {}", pipeline.config.debug_name);
  } else {
    eidolon_trace("created graphics pipeline");
  }
  return handle;
}
...
handle_fence_t context_t::create_fence(const config_fence_t &config) {
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created fence {}", fence.config.debug_name);
  } else {
    eidolon_trace("created fence");
  }
  return handle;
}
...
handle_semaphore_t context_t::create_semaphore(
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created semaphore {}", semaphore.config.debug_name);
  } else {
    eidolon_trace("created semaphore");
  }
  return handle;
}
...
handle_command_pool_t context_t::create_command_pool(
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created command pool {}", command_pool.config.debug_name);
  } else {
    eidolon_trace("created command pool");
  }
  return handle;
}
...
handle_commandbuffer_t context_t::allocate_commandbuffer(
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("allocated commandbuffer {}",
...
handle_timer_t context_t::create_timer(const config_timer_t &config) {
  ...
  if (config.debug_name != "") {
    ...
    vkSetDebugUtilsObjectNameEXT(_vkb_device, &vk_debug_utils_object_name_info);
    eidolon_trace("created timer {}", timer.config.debug_name);
  } else {
    eidolon_trace("created timer");
  }
  return handle;
}
...
#ifndef eidolon_FORCE_WAIT_QUERY_RESULT
...
#endif  // eidolon_FORCE_WAIT_QUERY_RESULT
...
void context_t::cmd_bind_vertex_buffers(
  ...
  eidolon_profile();
  eidolon_assert(handle_buffers.size() == vk_offsets.size(),
